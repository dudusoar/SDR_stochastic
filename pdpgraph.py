...
class PDPGraph:
    def __init__(self, real_map, order_pairs_table, speed):
        '''
    ...
    def generate_time_matrix(self):
        '''
        build the time matrix for pdp based on the real distance matrix and speed
        '''
        num_nodes = len(self.N)
        ...
        for i in self.N:
            for j in self.N:
                if i != j:
                    distance = real_distance_matrix[self.pdp_to_real[i]][self.pdp_to_real[j]]
                    time_matrix[i][j] = (distance / self.speed)*60
                else:
                    time_matrix[i][j] = 0
...
if __name__ == "__main__":
    ...
    # real map
    real_map = RealMap(n_r=1, n_c=2, dist_function = np.random.uniform, dist_params = {'low': -2, 'high': 2})
    # demand table
    time_intervals = ((0, 10), (10, 20), (20, 30))
    demand_table = generate_demand_table(
        real_map.pairs, time_intervals,
        sample_dist=np.random.randint, sample_params={'low': 1, 'high': 2},
        demand_dist=np.random.poisson, demand_params={'lam': 1})
    # order pairs table
    order_pairs_table = generate_order_pairs(demand_table, real_map.coordinates)

    # pdp graph instances
    pdpGraph = PDPGraph(real_map, order_pairs_table,speed = 4)
