...
This directory contains tests for validating that all VRP-Toolkit tutorial notebooks can execute successfully.
...
Validate that all 7 tutorial notebooks in `vrp-toolkit/tutorials/` can:
- Execute without errors
- Complete within reasonable time limits
- Work with the current vrp-toolkit codebase
...
```bash
# Install vrp-toolkit in development mode
cd vrp-toolkit
pip install -e .
...
```bash
cd vrp-toolkit/tests/tutorials
...
**Solution**: Install vrp-toolkit in development mode:
```bash
cd vrp-toolkit
pip install -e .
```
...
1. **Dependency Check**: Verifies jupyter, nbconvert, and vrp-toolkit are installed
...
When adding a new tutorial to `vrp-toolkit/tutorials/`:
