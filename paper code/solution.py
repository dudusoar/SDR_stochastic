...
class PDPTWSolution:
    ...
    def __init__(self, instance, vehicle_capacity, battery_capacity, battery_consume_rate, routes, penalty_unvisit, penalty_delay):
        """
        Initialize PDPTWSolution object
        :param instance: PDPTWInstance object
        :param vehicle_capacity: vehicle capacity
        :param battery_capacity: battery capacity
        :param battery_consume_rate: battery consumption rate
        :param routes: given route list
        """
    ...
    def update_all(self):
        ...
        :param new_routes: new route list
        """
        self.initialize_routes()
        self.calculate_all()
        self.update_visit_record()
    ...
    def calculate_all(self):
        """
        Calculate all state variables and objective function values
        """
        for vehicle_id in range(self.num_vehicles):
    ...
    def calculate_battery_capacity_levels(self, vehicle_id):
        """
        Calculate battery level and capacity level at each node on the route for the specified vehicle
    ...
    def calculate_arrival_leave_wait_times(self, vehicle_id):
        """
        Calculate arrival time, departure time, and waiting time at each node on the route for the specified vehicle
        :param vehicle_id: vehicle ID
        """
        route = self.routes[vehicle_id]
    ...
    def calculate_travel_delay_wait_times(self, vehicle_id):
        """
        Calculate total travel time, total delay time, and total waiting time for the specified vehicle
        :param vehicle_id: vehicle ID
        """
        route = self.routes[vehicle_id]
        leave_times = self.route_leave_times[vehicle_id]
    ...
    def objective_function(self):
        ...
        :return: Objective function value (total travel time + total delay time)
        """
    ...
    def get_selected_vehicles(self):
        """
        Get selected vehicle list
        :return: Selected vehicle ID list
        Paths of unselected vehicles are initialized as [0,0]
        """
        selected_vehicles = []
        for vehicle_id in range(self.num_vehicles):
    ...
    def check_capacity_constraint(self, selected_vehicles):
        ...
        :param selected_vehicles: Selected vehicle ID list
        """
    ...
    def check_pickup_delivery_order(self, selected_vehicles):
        ...
        :param selected_vehicles: Selected vehicle ID list
        :return: True if pickup-delivery order constraint is satisfied, False otherwise
        """
        for vehicle_id in selected_vehicles:
            route = self.routes[vehicle_id]
            visited_pickups = set()
    ...
    def plot_solution(self, vehicle_ids=None):
        ...
        :param vehicle_ids: List of vehicle IDs to plot routes for (optional), defaults to None, meaning plot all selected vehicles
        """
        selected_vehicles = self.get_selected_vehicles()
        if not selected_vehicles:
        ...
        # Populate location_orders dictionary
