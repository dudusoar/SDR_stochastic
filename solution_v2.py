        ...
        for k in range(self.v):
            ...
            if len(route) > 0:  # 车辆k被选择
                ...
                for i in range(1, len(route)):
                    prev_node = route[i - 1]
                    node = route[i]
                    self.arrival_times[k, i] = self.arrival_times[k, i - 1] + self.params['distance_matrix'][prev_node][
                        node]
                    if node in self.params['D'] and self.arrival_times[k, i] > self.params['l_i'][node]:
                        self.total_delay += self.arrival_times[k, i] - self.params['l_i'][node]
                    self.total_time += self.params['distance_matrix'][prev_node][node]
